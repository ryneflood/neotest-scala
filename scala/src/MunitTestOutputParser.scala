...
import neotest.MunitTestOutputParser.stripColorCodes
import neotest.MunitTestOutputParser.isPassedTest
import neotest.MunitTestOutputParser.isFailedTest

object MunitTestOutputParser:
...
private def splitIntoSuites(testSuiteNames: List[String])(
    lines: List[String]
  ...
  lines.foldLeft(Map.empty[String, List[String]]) { (acc, line) =>
    ...
    if isTestSuite(testSuiteNames, line) then
      acc.updated(stripColorCodes(line).dropRight(1), List.empty)
    else {
